...
void parseJson(__global const charN *restrict jsonstr, int word_num, __global char *restrict output) {
  ...
  int index=0;
  int out_index=0;
  line_content = jsonstr[get_global_id(0)];
  #pragma unroll 16
  for (int j = 0 ;j < N;j++) {
    ...
    if (temp[j]==':') {
      ...
      fieldIndex++;
          ...
          else if (data_mode==2 && temp[j] != '"'){
             vals[fieldIndex-1][i]= temp[j] ; i++;
          }
	  else if (data_mode==2 && temp[j] == '"'){
            data_mode=0;
            data_size[fieldIndex-1]=i;
          }
        }
  }
    
  int currentUnsafeRowPosition = get_global_id(0)*56;
  #pragma unroll
